...
sys.exit(1)")" || die "no dev release found"
        ...
        # Use awk to find the first release where prerelease is true and
        # capture its tag_name. awk is available on Termux and avoids the
        # field-ordering and pipefail issues of the grep approach.
