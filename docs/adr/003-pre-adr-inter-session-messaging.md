...
When running multiple `dev` sessions each hosting an AI agent (Claude Code, OpenCode, PiAgent, etc.), there is no built-in mechanism for one agent to send a message to another. The use case that drove this: an agent in session `ferrcad-poc` needed to delegate a task to an agent in `manta-manager-python`.

### What was tried (issue #69)

**Attempt 1 — `dev run-in`:**
```bash
dev run-in manta-manager-python "echo hello"
```
...
```bash
dev run-in "manta-manager-python:0.0" "echo hello"
```
...
```bash
tmux list-windows -t manta-manager-python     # → "1: opencode* (1 panes)"
tmux list-panes -t manta-manager-python:1 -F "#{pane_index}"  # → "1"
tmux send-keys -t manta-manager-python:1 "review this issue" Enter
```
...
```
dev run-in --send-keys manta-manager-python "message"
```
...
```
dev run-in manta-manager-python "echo hello"   # now sends keys (visible)
dev run-in --capture manta-manager-python "cmd" # old behaviour
```
